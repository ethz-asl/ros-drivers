...
#	ifdef MVIMPACT_H_
		...
		public:
			...
			/// \b EXAMPLE
			///
			/// In case of error \b -2102 this function would return \b DMR_DRV_ALREADY_IN_USE, which
...
#	endif // #ifdef MVIMPACT_H_
	...
	/// \b EPropertyHandling objects can be written to catch all property module related
	/// exceptions.
	class EPropertyHandling : public ImpactAcquireException
	//-----------------------------------------------------------------------------
	{
	public:
		/// \brief Creates a new \b EPropertyHandling object.
	...
	{
	protected:
		/// \brief Creates a new \b EComponent object.
...
protected:
	...
	/// of \b mvIMPACT::acquire::ComponentAccess
	HOBJ m_hObj;
...
public:
	...
	///
	/// \return A new \b mvIMPACT::acquire::Component object
	Component nextSibling( void ) { return ++*this; }
	...
	std::string flagsAsString(	/// A user definable string to separate the individual flags.
								...
								/// like this: \b 'cfWriteAccess | cfReadAccess'
	...
	///\endxmlonly
	///
	/// To find out how many \b mvIMPACT::acquire::Components are selected by the current one call 
	...
	///\endxmlonly
	///
	/// To find out how many \b mvIMPACT::acquire::Components are selecting the current one call 
...
//-----------------------------------------------------------------------------
{
	...
	/// can store with a call to the \b mvIMPACT::acquire::Propertys corresponding
	/// ::read or ::write.
...
/// to do with a property or any other component the function \b "mvIMPACT::acquire::Component::flags() const"
/// can be called.
///
...
public:
	...
	std::string readSArray(	/// The format string telling the function how to format the result. If left empty the property uses its standard way of converting its data into a string
							...
							int startIndex = 0,
							/// The index of the last of the desired values. If \b INT_MAX is passed, every
...
public:
									...
									int index = 0 ) const
	{
		...
		/// therefore this function should be called whenever all entries are required.
		/// \return A const reference to the calling property.
		const EnumPropertyF& getTranslationDictValues( std::vector<ZYX>& sequence ) const
		{
			TPROPHANDLING_ERROR result = PROPHANDLING_NO_ERROR;
...
/// To use a normal int (not enumerated) properties use the \b PropertyI type.
template<class ZYX>
class EnumPropertyI : public Property
//-----------------------------------------------------------------------------
{
public:
	/// \brief Constructs a new unbound \b EnumPropertyI object.
	explicit EnumPropertyI() {}
	/// \brief Constructs a new \b EnumPropertyI object.
									...
									int index = 0 ) const
	{
		...
		/// \note
		/// This function is much more efficient than calling
		/// \b EnumPropertyI::getTranslationDictValue multiple times and
		/// therefore this function should be called whenever all entries are required.
		/// \return A const reference to the calling property.
...
{
public:
	/// \brief Constructs a new unbound \b EnumPropertyI64 object.
	explicit EnumPropertyI64() {}
	/// \brief Constructs a new \b EnumPropertyI64 object.
									...
									int index = 0 ) const
	{
		...
		/// \note
		/// This function is much more efficient than calling
		/// \b EnumPropertyI64::getTranslationDictValue multiple times and
		/// therefore this function should be called whenever all entries are required.
		/// \return A const reference to the calling property.
		const EnumPropertyI64& getTranslationDictValues( std::vector<ZYX>& sequence ) const
		{
			TPROPHANDLING_ERROR result = PROPHANDLING_NO_ERROR;
			const unsigned int size = dictSize();
			if( size == 0 )
...
public:
	...
	/// by a property before calling the function \b OBJ_SetBinary() this function can be used.
	///
	/// \sa \b mvIMPACT::acquire::PropertyS::readBinary, \n
	///     \b mvIMPACT::acquire::PropertyS::writeBinary, \n
	///     \b mvIMPACT::acquire::PropertyS::binaryDataBufferMaxSize
	...
	///\endxmlonly
	///
	/// \sa \b mvIMPACT::acquire::ComponentAccess::readBinary, \n
	///     \b mvIMPACT::acquire::ComponentAccess::binaryDataBufferSize, \n
	///     \b mvIMPACT::acquire::ComponentAccess::binaryDataBufferMaxSize
	///
	void
#else
	/// \brief Writes a block of binary data to one entry of the property.
...
public:
	...
	/// \return A new \b mvIMPACT::acquire::UserDataEntry instance, that can be user to store user specific data
...
///
/// \note
/// \b IMPORTANT: Whenever a device is closed via a call to \b mvIMPACT::acquire::Device::close
...
///
/// \note
/// \b IMPORTANT: Whenever the last instance to a \b DeviceManager object get's
...
PYTHON_ONLY(%immutable;)
	...
	/// \brief A string property \b (read-only) containing the auto load order of feature settings that is executed during
	...
	/// \brief An integer property that defines the number of \b mvIMPACT::acquire::Request objects to be created when
	...
	PropertyI resultQueueCount;
		...
		/// an exclamation mark ('!') in it's name. \b NEVER rely on that! These exclamation marks
	...
	/// \brief An enumerated integer property \b (read-only) that can be used to query the granted access
...
#if !defined(DOXYGEN_SHOULD_SKIP_THIS) && !defined(WRAP_ANY)
		// Returns the internal handle to the device manager created via \b DMR_Init().
...
/// instance of \b mvIMPACT::acquire::DeviceComponentLocator.
...
//-----------------------------------------------------------------------------
{
	...
	/// \brief Specifies the driver interface list providing access to the drivers \b H ardware \b R eal-\b T ime \b C ontroller (HRTC).
	///
...
public:
	...
	explicit DeviceComponentLocator(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from
	...
	/// \return The unique identifier of the new search base.
	HLIST bindSearchBaseList(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from
								/// a \b mvIMPACT::acquire::DeviceManager object. 
								Device* pDev,
								/// The type list to locate
								TDeviceListType deviceListType,
...
public:
...
#endif // #ifndef WRAP_DOTNET
	/// \brief Constructs a new \b mvIMPACT::acquire::ImageBufferDesc object with \a channelCount channel specific
	/// data elements.
	///
	...
	explicit ImageBufferDesc(	/// The desired pixel format for the new \b mvIMPACT::acquire::ImageBuffer
								/// object handled by this descriptor.
								TImageBufferPixelFormat pixelFormat,
								/// The width of the new \b mvIMPACT::acquire::ImageBuffer object handled
								/// by this descriptor.
								int width,
								/// The height of the new \b mvIMPACT::acquire::ImageBuffer object handled
								/// by this descriptor.
...
protected:
		...
		//-----------------------------------------------------------------------------
		{
			...
			ReferenceCountedData( Device* pDev, int requestNr ) : m_requestNr(requestNr), m_imageBufferDesc(1), m_pDev(pDev),
				m_infoIterator(), m_chunkCounterValues(), m_chunkTimerValues(), m_refCnt(1) {}
			~ReferenceCountedData()
			{
...
protected:
	...
	//-----------------------------------------------------------------------------
	{
		...
		DeviceComponentLocator locator(m_pRefData->m_pDev, dltInfo);
		locator.bindSearchBase( locator.searchbase_id(), "CurrentRequestLayout" );
		return locator;
	}
public:
	/// \brief Constructs a new \b mvIMPACT::acquire::Request from
	/// and exisiting one.
	explicit Request(	/// A constant reference to the \b mvIMPACT::acquire::Request object, this object shall be created from
	...
	///
	/// \note
	/// Please do \b NEVER work with old references to this structure. Please see
	/// the comment made under \b mvIMPACT::acquire::Request::getResult, as the same
	/// applies for the reference to this \b mvIMPACT::acquire::ImageBufferDesc.
...
#	endif
	...
	unsigned int getChunkCounterCount( void ) const { return static_cast<unsigned int>(m_pRefData->m_chunkCounterValues.size()); }
		...
		/// \brief Returns a 64 bit integer property \b (read-only) defining the selected couter value at the time
	...
	/// \brief Returns a 64 bit integer property \b (read-only) defining the selected timer value at the time
	/// of the internal frame start event for this request.
	///
	///\xmlonly
	/// <mvWrapperDoc><htmlinclude ref="code_fragment_start_cpp.html"/></mvWrapperDoc>
	...
	/// - \b mvIMPACT::acquire::DMR_NO_ERROR if successful.
	/// - A negative error code of type \b mvIMPACT::acquire::TDMR_ERROR otherwise.
	int configure( void ) { return DMR_ImageRequestConfigure( m_pRefData->m_pDev->hDrv(), m_pRefData->m_requestNr, 0, 0 ); }
	...
	/// - \b mvIMPACT::acquire::DMR_NO_ERROR if successful.
	/// - A negative error code of type \b mvIMPACT::acquire::TDMR_ERROR otherwise.
	int unlock( void ) { return DMR_ImageRequestUnlock( m_pRefData->m_pDev->hDrv(), m_pRefData->m_requestNr ); }
	...
	/// pRequest->attachBuffer( getBufferFromSomewhere(), getBufferSizeFromSomewhere() );
	///\endcode
	///
	/// Instead if this:
	///
	...
	/// \ref ExampleApplications_subsection_CaptureToUserMemory_source and have a look at the
...
PYTHON_ONLY(%immutable;)
	...
	/// \brief An integer property \b (read-only) containing a frame identifier.
	///
	...
	///
	/// \note This property will only contain meaningful data if the device supports \b HRTC and a program is running and writing data to
	/// the property OR if a device supports the transmission of a unique identifier for an image. Currently HRTC programs are supported by \b mvBlueFOX
	...
	/// \brief An integer property \b (read-only) containing the number of images captured since the driver has been initialised including
	/// the current image.
	PropertyI infoFrameNr;
	/// \brief An integer property \b (read-only) containing a timestamp (in us) defining the time the device started the exposure of the
	/// image associated with this \b mvIMPACT::acquire::Request object.
	///
	...
	/// \brief An integer property \b (read-only) containing the time the transaction of this image has been delayed (in us) because
	...
	/// \brief An integer property \b (read-only) containing the state of all the IO lines at the end of the frame
	...
	/// \brief A 64 bit integer property \b (read-only) containing the X-offset of the image in pixel as returned in the
	/// chunk data attached to the image.
	///
	/// \note
	/// For additional information about the chunk data format please refer to \ref ImageAcquisition_section_chunk.
	PropertyI64 chunkOffsetX;
	/// \brief A 64 bit integer property \b (read-only) containing the Y-offset of the image in pixel as returned in the
	/// chunk data attached to the image.
	///
	/// \note
	/// For additional information about the chunk data format please refer to \ref ImageAcquisition_section_chunk.
	PropertyI64 chunkOffsetY;
	/// \brief A 64 bit integer property \b (read-only) containing the width of the image in pixel as returned in the
	/// chunk data attached to the image.
	///
	/// \note
	/// For additional information about the chunk data format please refer to \ref ImageAcquisition_section_chunk.
	PropertyI64 chunkWidth;
	/// \brief A 64 bit integer property \b (read-only) containing the height of the image in pixel as returned in the
	/// chunk data attached to the image.
	///
	/// \note
	/// For additional information about the chunk data format please refer to \ref ImageAcquisition_section_chunk.
	PropertyI64 chunkHeight;
	/// \brief A 64 bit integer property \b (read-only) containing the pixel format of the image as returned in the
	/// chunk data attached to the image.
	///
	...
	/// \brief A 64 bit integer property \b (read-only) containing the minimum value of dynamic range of the image as
	/// returned in the chunk data attached to the image.
	///
	/// \note
	/// For additional information about the chunk data format please refer to \ref ImageAcquisition_section_chunk.
	PropertyI64 chunkDynamicRangeMin;
	/// \brief A 64 bit integer property \b (read-only) containing the maximum value of dynamic range of the image as
	/// returned in the chunk data attached to the image.
	///
	/// \note
	/// For additional information about the chunk data format please refer to \ref ImageAcquisition_section_chunk.
	PropertyI64 chunkDynamicRangeMax;
	/// \brief A 64 bit integer property \b (read-only) containing the timestamp value of the internal frame start
	/// signal of the image as returned in the chunk data attached to the image.
	///
	/// \note
	/// For additional information about the chunk data format please refer to \ref ImageAcquisition_section_chunk.
	PropertyI64 chunkTimestamp;
	/// \brief A 64 bit integer property \b (read-only) containing the status of all I/O lines at the time of the
	...
	/// again. However, once a \b mvIMPACT::acquire::Request has been returned to the user, it's image buffer
	...
	/// \b mvIMPACT::acquire::FunctionInterface::imageRequestConfigure, \n
	/// \b mvIMPACT::acquire::Request::configure, \n
	/// \b mvIMPACT::acquire::Request::imageDataSize, \n
	...
	/// \brief An integer property \b (read-only) containing the offset (in bytes) to the next line of each channel beloning to the current image.
	///
	/// \note
	/// This property will store \a imageChannelCount values. These can be queried one by one using the \a index parameter of the function
	/// \b mvIMPACT::acquire::PropertyI::read() or in a single call by using the overloaded \a read function accepting a reference to a
	...
	/// \brief An string property \b (read-only) containing the string descriptors of each channel belonging to the current image.
	///
	/// \note
	/// This property will store \a imageChannelCount values. These can be queried one by one using the \a index parameter of the function
	/// \b mvIMPACT::acquire::PropertyI::read() or in a single call by using the overloaded \a read function accepting a reference to a
	...
	/// \brief An integer property \b (read-only) containing the X-offset of the image in pixel.
	///
	/// \note
	...
	/// \brief An integer property \b (read-only) containing the Y-offset of the image in pixel.
	///
	/// \note
	/// This feature will be supported by devices using an mvIMPACT Acquire driver greater or equal version 1.10.92.
	PropertyI imageOffsetY;
	/// An integer property \b (read-only) containing the width of the image in pixel.
	PropertyI imageWidth;
	/// \brief An integer property \b (read-only) containing the total width of the image in pixel if this buffer is part of a larger image.
	///
	/// \note
	...
	/// \brief An integer property \b (read-only) containing the height of the image in pixel.
	PropertyI imageHeight;
	/// \brief An integer property \b (read-only) containing the total height of the image in pixel.
	///
	/// \note
	/// This feature will be supported by devices using an mvIMPACT Acquire driver greater or equal version 1.11.50. Not
	/// every device driver will be capable of dealing with multiple buffers forming a single image. If a device driver can
...
{
public:
	explicit ImageRequestControl(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a 
...
PYTHON_ONLY(%immutable;)
	...
	/// will be sent after the acquisition.
...
public:
#if defined(WRAP_DOTNET) || defined(DOTNET_ONLY_CODE)
	virtual Request* createRequest(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from
									/// a \b mvIMPACT::acquire::>DeviceManager object.
									Device* pDev,
...
public:
	...
	/// \sa
	/// \b Device::open
	explicit FunctionInterface(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from
								/// a \b mvIMPACT::acquire::>DeviceManager object.
								Device* pDev,
	...
	/// \brief Constructs a new \b mvIMPACT::acquire::FunctionInterface from
	/// and exisiting one.
	explicit FunctionInterface(	/// A constant reference to the \b mvIMPACT::acquire::FunctionInterface object, this object shall be created from
	...
	///
	/// This function creates a new \b mvIMPACT::acquire::ImageRequestControl based on an existing one. New \b mvIMPACT::acquire::ImageRequestControls can only be derived
	/// from \b mvIMPACT::acquire::ImageRequestControls that already exist. When the driver has been initialised there will at least be
	...
	///
	/// \note
	/// See all comments made under \b mvIMPACT::acquire::Component::FunctionInterface::createSetting for understanding the relationship between base and
	...
	int createImageRequestControl(	/// The name of the new \b mvIMPACT::acquire::ImageRequestControl object.
									const std::string& name,
									/// The name of the \b mvIMPACT::acquire::ImageRequestControl the new object shall be derived from.
									const std::string& parent = "Base",
									/// A pointer to a \b mvIMPACT::acquire::ComponentList object that will receive the ID of the newly created request control.
	...
	int createSetting(	/// The name of the setting to be created.
						...
						const std::string& parent = "Base",
						/// A pointer to a \b mvIMPACT::acquire::ComponentList object that will receive the ID of the newly created setting.
						/// This parameter can be 0 if the application is not interessted in that parameter.
	...
	/// \brief Returns a const pointer to the desired \b mvIMPACT::acquire::Request.
	///
	/// This function returns a pointer to the \b mvIMPACT::acquire::Request stored at \a requestNr
	/// in the internal array of requests.
	///\xmlonly
	/// <mvWrapperDoc><htmlinclude ref="code_fragment_start_cpp.html"/></mvWrapperDoc>
	///\endxmlonly
	///\htmlinclude code_fragment_start_cpp.html
	/// If \a requestNr refers to an invalid \b mvIMPACT::acquire::Request \n
	...
	Request* getRequest(	/// The number of the request to return
							int nr ) const
	{
		updateRequests();
	...
	/// - \b mvIMPACT::acquire::DMR_NO_ERROR if successful.
	/// - A negative error code of type \b mvIMPACT::acquire::TDMR_ERROR otherwise.
	int imageRequestReset(	/// The \b mvIMPACT::acquire::ImageRequestControl for which all
	...
	/// - \b mvIMPACT::acquire::DMR_NO_ERROR if successful.
	/// - A negative error code of type \b mvIMPACT::acquire::TDMR_ERROR otherwise.
	int imageRequestSingle( /// A pointer to the \b mvIMPACT::acquire::ImageRequestControl object
	...
	/// results.  \n \n When the result is negative the return value is an error code.
	/// The error code that is returned in most of the cases will be \b mvIMPACT::acquire::DEV_WAIT_FOR_REQUEST_FAILED.
	...
	/// get informations on how the capture buffers must be constructed. The most important parameters
	...
	/// \ref ExampleApplications_subsection_CaptureToUserMemory_source
	///
	/// \note
	...
	/// - \b mvIMPACT::acquire::DMR_NO_ERROR if successful.
	/// - A negative error code of type \b mvIMPACT::acquire::TDMR_ERROR otherwise.
	int getCurrentCaptureBufferLayout(	/// A reference to the \b mvIMPACT::acquire::ImageRequestControl object 
										...
										/// On a successful call this variable will receive a pointer to a
										/// \b mvIMPACT::acquire::Request object reflecting the current
										/// capture buffer layout.
										Request** ppRequest = 0,
										/// On a successful call this variable will receive the alignment
	...
	///
	/// When an application wants to provide capture buffers, this function will be needed in order to
	/// get informations on how the capture buffers must be constructed.
	///
	/// To find out more about capturing to user memory please refer to the application 
	/// \ref ExampleApplications_subsection_CaptureToUserMemory_source
	///
	...
	int getCurrentCaptureBufferLayout(	/// A reference to the \b mvIMPACT::acquire::ImageRequestControl object 
										...
										/// \b mvIMPACT::acquire::ImageRequestControl::setting.
										ImageRequestControl& imageRequestControl,
										/// On a successful call this variable will receive the size
										/// needed for capturing into user supplied buffers that use the
										/// current settings.
										int& size,
										/// On a successful call this variable will receive the alignment
	...
	/// \brief Check if \a requestNr specifies a valid \b mvIMPACT::acquire::Request
	/// \return  \n
...
PYTHON_ONLY(%immutable;)
	...
	/// \brief A string property \b (read-only) containing the name and the full path of the current log-file
	/// for this device.
	PropertyS logFile;
	/// \brief An integer property \b (read-only) containing the capture buffer alignment in bytes needed by this
	/// device driver.
	PropertyI captureBufferAlignment;
	/// \brief A string property \b (read-only) containing an array of full search pathes to lists which are recommended
...
public:
	...
	explicit Info(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
...
{
public:
	explicit BasicDeviceSettings(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
										Device* pDev,
...
public:
	...
	explicit ImageDestination(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
								Device* pDev,
								/// The name of the driver internal setting to access with this instance.
								/// A list of valid setting names can be obtained by a call to
								/// \b mvIMPACT::acquire::FunctionInterface::getAvailableSettings, new
...
PYTHON_ONLY(%immutable;)
	...
	PropertyF blueGain;
	/// \brief An enumerated integer property \b (read-only) containing the result of the last
...
//-----------------------------------------------------------------------------
/// \brief Properties for configuring settings belonging to a certain LUT(\b L ook\b U p \b T able)
...
PYTHON_ONLY(%immutable;)
	...
	/// \brief An enumerated integer property which can be used to configure the LUT(\b L ook \b U p \b T able) gamma mode.
	///
	...
	/// \b mvIMPACT::acquire::LUTParameters::gammaMode \b mvIMPACT::acquire::LUTgmLinearStart.
	///
...
public:
	...
	explicit ImageProcessing(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
								Device* pDev,
								/// The name of the driver internal setting to access with this instance.
								/// A list of valid setting names can be obtained by a call to
								/// \b mvIMPACT::acquire::FunctionInterface::getAvailableSettings, new
	...
	/// \brief Constructs a new \b mvIMPACT::acquire::ImageProcessing from
	/// and exisiting one.
	explicit ImageProcessing(	/// A constant reference to the \b mvIMPACT::acquire::ImageProcessing object, this object shall be created from
...
PYTHON_ONLY(%immutable;)
	...
	/// \brief An enumerated integer property which can be used to enable LUT (\b L ook \b U p \b T able) transformations.
	///
	/// Valid values for this property are defined by the enumeration \b mvIMPACT::acquire::TBoolean.
	PropertyIBoolean LUTEnable;
	/// \brief An enumerated integer property which can be used to configure the LUT (\b L ook \b U p \b T able) transformation mode.
	///
	...
	/// \brief An enumerated integer property which can be used to configure the LUT (\b L ook \b U p \b T able) interpolation mode.
	///
	...
	/// \brief An enumerated integer property which can be used to configure the LUT (\b L ook \b U p \b T able) implementation.
	///
	...
	/// \brief An enumerated integer property which can be used to configure the LUT (\b L ook \b U p \b T able) mapping mode
...
PYTHON_ONLY(%mutable;)
	...
	/// \brief Returns a reference to a set of user definable parameters to configure LUT (\b L ook \b U p \b T able) operations.
	///
	///\xmlonly
	/// <mvWrapperDoc><htmlinclude ref="code_fragment_start_cpp.html"/></mvWrapperDoc>
	///\endxmlonly
	...
	///\endxmlonly
	///
	/// \return A reference to one of the parameter sets for LUT (\b L ook \b U p \b T able) operations that
	/// can be altered by the user.
	...
	/// \brief Returns the number of available user definable parameter sets for LUT (\b L ook \b U p \b T able) operations.
...
public:
	explicit StatisticsBase(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a 
								/// \b mvIMPACT::acquire::DeviceManager object.
...
PYTHON_ONLY(%immutable;)
	...
	/// \brief An integer property \b (read-only) containing the time (in seconds) needed to process the image data
	/// internally.
	///
	...
	/// \brief An integer property \b (read-only) containing the time (in seconds) needed to convert the image data
	/// delivered by the hardware and/or active internal driver filter nodes to the format specified by \b mvIMPACT::acquire::ImageDestination::pixelFormat
	/// internally.
	///
	...
	/// \brief A float property \b (read-only) containing the average time (in seconds) a \b mvIMPACT::acquire::Request object
	/// spends in the request queue of the device.
	PropertyF queueTime_s;
	/// \brief An integer property \b (read-only) containing the number of images that have been
	...
	/// \brief A float property \b (read-only) containing the average amount of data missing in frames that haven't been captured completely since the
	/// \b mvIMPACT::acquire::Device has been opened OR since the last call to \b mvIMPACT::acquire::StatisticsBase::reset().
	///
	/// This value is a good indicator for bandwidth problems on the device to host connection.
	///
	...
	/// \brief A 64 bit integer property \b (read-only) containing the number of retransmit requests issued due to
...
public:
	...
	explicit Statistics(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
...
protected:
	...
	explicit SystemBase(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a 
							/// \b mvIMPACT::acquire::DeviceManager object.
...
PYTHON_ONLY(%immutable;)
	...
	///
	/// There are not too many good reasons to modify the default request count suggested by the device driver. Good reasons include:
	///
...
public:
	...
	explicit SystemSettings( /// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
...
/// with the \b mvIMPACT::acquire::dilDeviceSpecific interface layout.
///
...
#	ifndef IGNORE_MVGRABBER_SPECIFIC_DOCUMENTATION
		...
		public:
			/// \brief Constructs a new \b mvIMPACT::acquire::InfoBase object.
			explicit ImageMemoryManager(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from
											/// a \b mvIMPACT::acquire::DeviceManager object.
		...
		PYTHON_ONLY(%immutable;)
			...
			/// \brief An integer property \b (read-only) containing the number of individual blocks of DMA memory currently available when using 
...
#	if !defined(IGNORE_MVBLUEFOX_SPECIFIC_DOCUMENTATION) || !defined(IGNORE_MVBLUECOUGAR_SPECIFIC_DOCUMENTATION) || !defined(IGNORE_MVBLUELYNX_SPECIFIC_DOCUMENTATION)
		//-----------------------------------------------------------------------------
		/// \brief A more specific class to query information about a \b mvBlueDevice
		/// device and its driver(\b Device specific interface layout only).
		///
		/// This class contains a collection of properties providing various information about
		...
		public:
			...
			explicit InfoBlueDevice(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
										Device* pDev ) : InfoBase(pDev),
		...
		PYTHON_ONLY(%immutable;)
			/// \brief An integer property \b (read-only) containing the horizontal resolution
			...
			/// \brief An integer property \b (read-only) containing the vertical resolution
			/// of the camera sensor.
			///
			/// If the device has more than one sensor head, and all these sensor heads can be accessed using the current interface,
			/// this property will contain as many values as the devices offers sensor heads.
			PropertyI sensorYRes;
			/// \brief An enumerated integer property \b (read-only) containing the type of the sensor
			/// (color/mono/...)(if known).
			///
			...
			/// \brief An enumerated integer property \b (read-only) containing the type of sensor chip of the
			/// camera(if known).
			///
...
#	ifndef IGNORE_MVBLUECOUGAR_SPECIFIC_DOCUMENTATION
		//-----------------------------------------------------------------------------
		/// \brief A more specific class to query information about a \b mvBlueCOUGAR
		/// or \b mvBlueLYNX-M7 device and its driver(\b Device specific interface layout only).
		///
		/// This class contains a collection of properties providing various information about
		...
		public:
			...
			explicit InfoBlueCOUGAR(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
		...
		PYTHON_ONLY(%immutable;)
			...
			///
			/// \note
			/// This feature is currently not supported by \b mvBlueCOUGAR-S devices.
			...
			/// \brief An integer property \b (read-only) providing information about the current connection state of a remote
			/// client.
			///
...
#	ifndef IGNORE_MVBLUEFOX_SPECIFIC_DOCUMENTATION
		//-----------------------------------------------------------------------------
		/// \brief A more specific class to query information about a \b mvBlueFOX
		/// device and its driver(\b Device specific interface layout only).
		///
		/// This class contains a collection of properties providing various information about
		...
		public:
			...
			explicit InfoBlueFOX(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
		...
		PYTHON_ONLY(%immutable;)
			...
			/// \brief An integer property \b (read-only) containing a bit mask containing information
			...
			PropertyI64 deviceSensorRevision;
			/// \brief An integer property \b (read-only) containing the size of the user EEPROM that can be accessed
			/// using the I2C access features of \b mvIMPACT::acquire::I2CControl.
			///
			/// \note
			...
			/// \brief A float property \b (read-only) containing the temperature of the selected temperature sensor
...
#	endif // #ifdef DOTNET_ONLY_CODE
	...
	//-----------------------------------------------------------------------------
	/// \brief A class to represent one step of a real time control(\b RTCtr) program
...
#		endif // #ifdef DOXYGEN_SHOULD_SKIP_THIS
	...
	public:
		/// \brief Constructs a new \b mvIMPACT::acquire::RTCtrProgram from
		/// and exisiting one.
		explicit RTCtrProgram(	/// A constant reference to the \b mvIMPACT::acquire::RTCtrProgram object, this object shall be created from
...
#	endif // #ifndef WRAP_PYTHON (In Python, object assignment amounts to just a reference count increment anyhow; you need to call the constructor or possibly some slice operation to make a true copy)
	...
	PYTHON_ONLY(%immutable;)
		...
		PropertyS filename;
		/// \brief A string property \b (read-only) containing information
...
#	endif // #ifdef DOTNET_ONLY_CODE
		...
		/// property \b mvIMPACT::acquire::RTCtrProgram::filename
		///
	...
	/// wait for(\b Device specific interface layout only)(\b deprecated).
	///
	/// \note
	...
	public:
#	ifndef WRAP_PYTHON
			/// \brief Returns a const reference to the \b mvIMPACT::acquire::EventData structure of this event.
			///
...
#	endif // #	ifndef WRAP_PYTHON
	PYTHON_ONLY(%immutable;)
		/// \brief An enumerated integer property \b (read-only) defining the mode this event
...
#	endif // #ifdef DOTNET_ONLY_CODE
	...
	/// (\b Device specific interface layout only)(\b deprecated).
	///
	/// \note
	/// This class has been declared \b deprecated and will be removed in future versions
	/// of this interface. A more flexible way of getting informed about changes in driver features
	...
	/// objects and functions(\b Device specific interface layout only)(\b deprecated).
	///
	/// \note
	/// This class has been declared \b deprecated and will be removed in future versions
	/// of this interface. A more flexible way of getting informed about changes in driver features
...
#	ifndef WRAP_PYTHON
			...
			explicit MVIMPACT_DEPRECATED_CPP( EventSubSystem(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
																Device* pDev ) );
#	endif // #ifndef WRAP_PYTHON
		...
		explicit EventSubSystem(	/// A reference to an existing \b mvIMPACT::acquire::EventSubSystem.
...
#	ifndef WRAP_PYTHON
			...
			/// Internal data is reference counted, thus modifying an event obtained from \a src will also
			/// modify the same event obtained from this newly constructed class.
			EventSubSystem& operator=(	/// A reference to an existing \b mvIMPACT::acquire::EventSubSystem.
...
#	endif // #ifdef DOXYGEN_SHOULD_SKIP_THIS
	...
	/// outputs of a device(\b Device specific interface layout only).
	///
	/// \note This class will only be available if \b mvIMPACT::acquire::Device::interfaceLayout is set to
	/// \b mvIMPACT::acquire::dilDeviceSpecific before the device is opened.
...
#	endif // DOXYGEN_SHOULD_SKIP_THIS
	public:
		/// \brief Constructs a new \b mvIMPACT::acquire::IOSubSystem from
		/// and exisiting one.
		explicit IOSubSystem(	/// A constant reference to the \b mvIMPACT::acquire::IOSubSystem object, this object shall be created from
...
#	endif // #ifndef WRAP_PYTHON (In Python, object assignment amounts to just a reference count increment anyhow; you need to call the constructor or possibly some slice operation to make a true copy)
		/// \brief Returns a pointer to a \b mvIMPACT::acquire::PulseStartConfiguration associated with
		/// this device.
		///
		///\xmlonly
		/// <mvWrapperDoc><htmlinclude ref="code_fragment_start_cpp.html"/></mvWrapperDoc>
		...
		/// \brief Returns the number of \b mvIMPACT::acquire::PulseStartConfigurations available for the \b mvIMPACT::acquire::Device
		...
		/// \brief Returns a pointer to a \b mvIMPACT::acquire::RTCtrProgram associated with
		/// this device.
		///
		///\xmlonly
		/// <mvWrapperDoc><htmlinclude ref="code_fragment_start_cpp.html"/></mvWrapperDoc>
		...
		/// \brief Returns the number of \b mvIMPACT::acquire::RTCtrProgram s available for the \b mvIMPACT::acquire::Device
		/// associated with this object.
		unsigned int RTCtrProgramCount( void ) const { return static_cast<unsigned int>(m_pRefData->m_RTCtrPrograms.size()); }
		/// \brief Returns the number of \b mvIMPACT::acquire::DigitalInput s available for the \b mvIMPACT::acquire::Device
		/// associated with this object.
		...
		/// \brief Returns the number of digital outputs available for the \b mvIMPACT::acquire::Device
		/// associated with this object.
...
#	ifndef IGNORE_MVBLUEFOX_SPECIFIC_DOCUMENTATION
		//-----------------------------------------------------------------------------
		/// \brief A class to handle the digital inputs and outputs for \b mvBlueFOX
		/// USB cameras(\b Device specific interface layout only).
		///
		///\xmlonly
		/// <mvWrapperDoc><htmlinclude ref="code_fragment_start_cpp.html"/></mvWrapperDoc>
		///\endxmlonly
		...
		public:
			...
			explicit IOSubSystemBlueFOX(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
...
#	if !defined(IGNORE_MVGRABBER_SPECIFIC_DOCUMENTATION) || !defined(IGNORE_MVBLUECOUGAR_SPECIFIC_DOCUMENTATION)
		...
		///\htmlinclude code_fragment_start_cpp.html
		///
		/// \b EXAMPLES(lists all available digital inputs and outputs and their description): 
		///\code
		/// #include <mvIMPACT_CPP/mvIMPACT_acquire.h>
		/// #include <iostream>
		///
		...
		public:
			...
			explicit IOSubSystemCommon(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
...
#		endif // #ifdef DOXYGEN_SHOULD_SKIP_THIS
		public:
			...
			explicit OutputSignalGeneratorBlueDevice(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
			...
			/// and exisiting one.
			explicit OutputSignalGeneratorBlueDevice(	/// A constant reference to the \b mvIMPACT::acquire::OutputSignalGeneratorBlueDevice object, this object shall be created from
...
#		endif // #ifndef WRAP_PYTHON (In Python, object assignment amounts to just a reference count increment anyhow; you need to call the constructor or possibly some slice operation to make a true copy)
			...
			/// - \b mvIMPACT::acquire::DMR_NO_ERROR if successful.
			/// - A negative error code of type \b mvIMPACT::acquire::TDMR_ERROR otherwise.
			int undefineSignal(	/// A pointer to a \b mvIMPACT::acquire::DigitalOutput object
			...
			bool canCreateExposeActiveSignal(	/// A pointer to a \b mvIMPACT::acquire::DigitalOutput object
												/// obtained from an instance of the class \b mvIMPACT::acquire::IOSubSystemCommon
												/// for this device.
			...
			bool canInvertSignal(	/// A pointer to a \b mvIMPACT::acquire::DigitalOutput object
									/// obtained from an instance of the class \b mvIMPACT::acquire::IOSubSystemCommon
									/// for this device.
									DigitalOutput* pOutput ) const
			{
			...
			TDeviceDigitalOutputMode getOutputMode(	/// A pointer to a \b mvIMPACT::acquire::DigitalOutput object
													/// obtained from an instance of the class \b mvIMPACT::acquire::IOSubSystemCommon
													/// for this device.
													DigitalOutput* pOutput ) const
			{
			...
			bool isOutputModeActive(	/// A pointer to a \b mvIMPACT::acquire::DigitalOutput object
										/// obtained from an instance of the class \b mvIMPACT::acquire::IOSubSystemCommon
										/// for this device.
										DigitalOutput* pOutput,
			...
			bool isSignalInverted(	/// A pointer to a \b mvIMPACT::acquire::DigitalOutput object
									/// obtained from an instance of the class \b mvIMPACT::acquire::IOSubSystemCommon
									/// for this device.
									DigitalOutput* pOutput ) const
			{
			...
			///
			/// \return An error code of type \b mvIMPACT::acquire::TDMR_ERROR otherwise.
			int setOutputMode(	/// A pointer to a \b mvIMPACT::acquire::DigitalOutput object
								/// obtained from an instance of the class \b mvIMPACT::acquire::IOSubSystemCommon
								/// for this device.
								DigitalOutput* pOutput,
...
#	ifndef IGNORE_MVGRABBER_SPECIFIC_DOCUMENTATION
		...
		/// devices(\b Device specific interface layout only).
		///
		/// This class provides a convenient basic access to the digital inputs and 
		...
		public:
			...
			/// \brief Returns the number of available HD output pins for the \b mvIMPACT::acquire::Device
			/// associated with this object.
			unsigned int getHDOutputCount( void ) const { return static_cast<unsigned int>(m_pRefData->m_vHDOutputs.size()); }
			/// \brief Returns a pointer to a \b mvIMPACT::acquire::SyncOutput object that represents
			/// a HD output for this device.
			///
			/// \a outputNr \b MUST be a valid number ( larger or equal than 0 and smaller than the
			...
			/// \brief Returns the number of available VD output pins for the \b mvIMPACT::acquire::Device
			/// associated with this object.
		...
		PYTHON_ONLY(%immutable;)
			...
			/// desired output pin. Then it's property \b mvIMPACT::acquire::SyncOutput::frequency_Hz is
...
#		endif // #ifdef DOXYGEN_SHOULD_SKIP_THIS
		public:
			...
			/// \brief Constructs a new \b mvIMPACT::acquire::OutputSignalGeneratorFrameGrabber from
			/// and exisiting one.
			explicit OutputSignalGeneratorFrameGrabber(	/// A constant reference to the \b mvIMPACT::acquire::OutputSignalGeneratorFrameGrabber object, this object shall be created from
...
#		endif // #ifndef WRAP_PYTHON (In Python, object assignment amounts to just a reference count increment anyhow; you need to call the constructor or possibly some slice operation to make a true copy)
			...
			/// - \b mvIMPACT::acquire::DMR_NO_ERROR if successful.
			/// - A negative error code of type \b mvIMPACT::acquire::TDMR_ERROR otherwise.
			int definePulse(	/// A pointer to a \b mvIMPACT::acquire::DigitalOutput object
			...
			/// - \b mvIMPACT::acquire::DMR_NO_ERROR if successful.
			/// - A negative error code of type \b mvIMPACT::acquire::TDMR_ERROR otherwise.
			int definePulseSequence(	/// A pointer to a \b mvIMPACT::acquire::DigitalOutput object
										/// obtained from an instance of the class \b mvIMPACT::acquire::IOSubSystemFrameGrabber
										/// for this device.
										DigitalOutput* pOutput,
...
#		ifndef WRAP_DOTNET
				...
				/// - \b mvIMPACT::acquire::DMR_NO_ERROR if successful.
				/// - A negative error code of type \b mvIMPACT::acquire::TDMR_ERROR otherwise.
				int getValidSignalsForPassThroughModes(	/// A pointer to a \b mvIMPACT::acquire::DigitalOutput object
														/// obtained from an instance of the class \b mvIMPACT::acquire::IOSubSystemFrameGrabber
														/// for this device.
														DigitalOutput* pOutput,
...
#		endif // WRAP_DOTNET
			...
			unsigned int getValidPassThroughSignalCount(	/// A pointer to a \b mvIMPACT::acquire::DigitalOutput object
															/// obtained from an instance of the class \b mvIMPACT::acquire::IOSubSystemFrameGrabber
															/// for this device.
															DigitalOutput* pOutput ) const
			{
			...
			/// <mvWrapperDoc><htmlinclude ref="code_fragment_end.html"/></mvWrapperDoc>
			///\endxmlonly
			TDigitalSignal getValidPassThroughSignalValue(	/// A pointer to a \b mvIMPACT::acquire::DigitalOutput object
															/// obtained from an instance of the class \b mvIMPACT::acquire::IOSubSystemFrameGrabber
															/// for this device.
															DigitalOutput* pOutput,
			...
			/// <mvWrapperDoc><htmlinclude ref="code_fragment_end.html"/></mvWrapperDoc>
			///\endxmlonly
			std::string getValidPassThroughSignalString(	/// A pointer to a \b mvIMPACT::acquire::DigitalOutput object
															/// obtained from an instance of the class \b mvIMPACT::acquire::IOSubSystemFrameGrabber
															/// for this device.
															DigitalOutput* pOutput,
															/// The index of the entry to read from the property. The find out the last valid value for this parameter
			...
			/// - \b mvIMPACT::acquire::DMR_NO_ERROR if successful.
			/// - A negative error code of type \b mvIMPACT::acquire::TDMR_ERROR otherwise.
			int definePassThroughSignal(	/// A pointer to a \b mvIMPACT::acquire::DigitalOutput object
											/// obtained from an instance of the class \b mvIMPACT::acquire::IOSubSystemFrameGrabber
											/// for this device.
											DigitalOutput* pOutput,
			...
			/// - \b mvIMPACT::acquire::DMR_NO_ERROR if successful.
			/// - A negative error code of type \b mvIMPACT::acquire::TDMR_ERROR otherwise.
			int defineLineScanPulse(	/// A pointer to a \b mvIMPACT::acquire::DigitalOutput object
										/// obtained from an instance of the class \b mvIMPACT::acquire::IOSubSystemFrameGrabber
										/// for this device.
										DigitalOutput* pOutput,
										...
										/// The width of the signal to generate if \a pulseStartEvent is \b mvIMPACT::acquire::dsoseSyncInRisingEdge,
										/// \b mvIMPACT::acquire::dsosePeriodically or
										/// \b mvIMPACT::acquire::dsoseSyncInFallingEdge and is 
										/// ignored otherwise.
										int width_pclk,
										/// The polarity of the signal to generate if \a pulseStartEvent is \b mvIMPACT::acquire::dsoseSyncInRisingEdge,
										/// \b mvIMPACT::acquire::dsosePeriodically or
										/// \b mvIMPACT::acquire::dsoseSyncInFallingEdge and is 
										/// ignored otherwise.
										///
...
#	ifndef IGNORE_MVBLUECOUGAR_SPECIFIC_DOCUMENTATION
		...
		/// \b mvBlueCOUGAR of \b mvBlueLYNX-M7 device(\b Device specific interface layout only).
		///
		/// This class provides a convenient basic access to the digital inputs and 
		...
		public:
			...
			explicit IOSubSystemBlueCOUGAR(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
...
#	endif // #ifndef IGNORE_MVBLUECOUGAR_SPECIFIC_DOCUMENTATION
	...
	/// connected to a video signal sink(e.g. a frame grabber)(\b Device specific interface layout only).
	///
	...
	public:
		...
		explicit Connector(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
							Device* pDev,
							/// The name of the driver internal setting to access with this instance.
							/// A list of valid setting names can be obtained by a call to
							/// \b mvIMPACT::acquire::FunctionInterface::getAvailableSettings, new
	...
	PYTHON_ONLY(%immutable;)
		...
		/// \brief A string property \b (read-only) containing a description for the video channel 
		/// currently selected by the property \b mvIMPACT::acquire::Connector::videoChannel
		///
...
#	endif // #ifdef DOTNET_ONLY_CODE
	...
	{
	public:
		explicit CameraSettingsBase(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
										Device* pDev,
										/// The name of the driver internal setting to access with this instance.
										/// A list of valid setting names can be obtained by a call to
										/// \b mvIMPACT::acquire::FunctionInterface::getAvailableSettings, new
...
#	ifndef IGNORE_MVVIRTUALDEVICE_SPECIFIC_DOCUMENTATION
		...
		public:
			...
			explicit CameraSettingsVirtualDevice(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
													Device* pDev,
													/// The name of the driver internal setting to access with this instance.
													/// A list of valid setting names can be obtained by a call to
													/// \b mvIMPACT::acquire::FunctionInterface::getAvailableSettings, new
...
#	ifndef IGNORE_MVV4L2_SPECIFIC_DOCUMENTATION
		...
		public:
			...
			explicit CameraSettingsV4L2Device(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
													Device* pDev,
													/// The name of the driver internal setting to access with this instance.
													/// A list of valid setting names can be obtained by a call to
													/// \b mvIMPACT::acquire::FunctionInterface::getAvailableSettings, new
...
#	if !defined(IGNORE_MVBLUEFOX_SPECIFIC_DOCUMENTATION) || !defined(IGNORE_MVBLUECOUGAR_SPECIFIC_DOCUMENTATION) || !defined(IGNORE_MVBLUELYNX_SPECIFIC_DOCUMENTATION)
		...
		///
		/// \sa
		/// \b mvIMPACT::acquire::RequestInfo, \n mvIMPACT::acquire::Request::getInfo
...
#		endif // #ifdef DOTNET_ONLY_CODE
		...
		public:
			...
			explicit CameraSettingsBlueDevice(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
												Device* pDev,
												/// The name of the driver internal setting to access with this instance.
												/// A list of valid setting names can be obtained by a call to
												/// \b mvIMPACT::acquire::FunctionInterface::getAvailableSettings, new
		...
		PYTHON_ONLY(%immutable;)
			...
			/// (\b A utomatic \b G ain \b C ontrol) is operated in.
			///
			...
			/// (\b A utomatic \b E xpose \b C ontrol) is operated in.
			///
			/// The values used to capture the image, which are directly influenced by an activated AGC or AEC
			/// (e.g. the gain or the exposure time) will be returned as part of the result of the
			/// image request.
...
#	ifndef IGNORE_MVBLUELYNX_SPECIFIC_DOCUMENTATION
		...
		public:
			...
			explicit CameraSettingsBlueLYNX( /// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
										Device* pDev,
										/// The name of the driver internal setting to access with this instance.
										/// A list of valid setting names can be obtained by a call to
										/// \b mvIMPACT::acquire::FunctionInterface::getAvailableSettings, new
...
#	if !defined(IGNORE_MVBLUEFOX_SPECIFIC_DOCUMENTATION) || !defined(IGNORE_MVBLUECOUGAR_SPECIFIC_DOCUMENTATION)
		//-----------------------------------------------------------------------------
		/// \brief A class to configure a HDR (\b H igh \b D ynamic \b R ange knee point
		/// (\b Device specific interface layout only).
		///
		/// \note
...
#		endif // #ifdef DOTNET_ONLY_CODE
		...
		//-----------------------------------------------------------------------------
		/// \brief A class to control the HDR (\b H igh \b D ynamic \b R ange) parameters of a device
		/// (\b Device specific interface layout only).
		///
		/// \note
		/// Not every device will offer the features provided in this class. Before accessing properties call
...
#		endif // #ifndef WRAP_PYTHON (In Python, object assignment amounts to just a reference count increment anyhow; you need to call the constructor or possibly some slice operation to make a true copy)
			/// \brief Returns a reference to a set of user definable parameters to configure a HDR(\b H igh \b D ynamic \b R ange knee point.
			///
			...
			///\endxmlonly
			///
			/// \return A reference to a set of user definable parameters to configure a HDR (\b H igh \b D ynamic \b R ange knee point.
		...
		PYTHON_ONLY(%immutable;)
			/// \brief An enumerated integer property which can be used to enable/disable HDR (\b H igh \b D ynamic \b R ange) mode.
			///
			/// Valid values for this property are defined by the enumeration \b mvIMPACT::acquire::TBoolean.
			PropertyIBoolean HDREnable;
			/// \brief An enumerated integer property which can be used to configure the HDR (\b H igh \b D ynamic \b R ange) mode to work with.
			///
			...
			/// \brief An integer property to define the number of knee points to work with when \b mvIMPACT::acquire::HDRControl::HDRMode is set
			/// to \b mvIMPACT::acquire::cHDRmUser.
...
#	ifndef IGNORE_MVBLUEFOX_SPECIFIC_DOCUMENTATION
		...
		public:
			...
			explicit CameraSettingsBlueFOX( /// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
											Device* pDev,
											/// The name of the driver internal setting to access with this instance.
											/// A list of valid setting names can be obtained by a call to
											/// \b mvIMPACT::acquire::FunctionInterface::getAvailableSettings, new
		...
		PYTHON_ONLY(%immutable;)
			...
			///
			/// If this property is set to \b mvIMPACT::acquire::aocOff
			/// the offset can be adjusted manually by modifying the \b mvIMPACT::acquire::CameraSettingsBlueFOX::offset_pc property.
			///
...
#	ifndef IGNORE_MVBLUECOUGAR_SPECIFIC_DOCUMENTATION
		//-----------------------------------------------------------------------------
		/// \brief \b mvBlueCOUGAR and \b mvBlueLYNX-M7 related camera settings
		/// (\b Device specific interface layout only).
		///
		...
		public:
			...
			explicit CameraSettingsBlueCOUGAR(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
												Device* pDev,
												/// The name of the driver internal setting to access with this instance.
												/// A list of valid setting names can be obtained by a call to
												/// \b mvIMPACT::acquire::FunctionInterface::getAvailableSettings, new
			...
			/// \brief Returns a const pointer to a \b mvIMPACT::acquire::TriggerControl object or NULL if the name passed to the function
...
#	ifndef IGNORE_MVGRABBER_SPECIFIC_DOCUMENTATION
		...
		public:
			...
			explicit CameraSettingsFrameGrabber(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
													Device* pDev,
													/// The name of the driver internal setting to access with this instance.
													/// A list of valid setting names can be obtained by a call to
													/// \b mvIMPACT::acquire::FunctionInterface::getAvailableSettings, new
...
#		endif // #ifdef DOTNET_ONLY_CODE
		...
		public:
			...
			/// \b %ALLUSERS%\\Documents\\MATRIX VISION\\mvIMPACT acquire\\CameraFiles (or \b "%MVIMPACT_ACQUIRE_DATA_DIR%\\CameraFiles" which will point to the same folder), under Linux&reg;
			/// this directory will be \b /etc/matrix-vision/mvimpact-acquire/camerafiles while under
...
#		endif // #ifdef DOTNET_ONLY_CODE
		...
		public:
		PYTHON_ONLY(%immutable;)
			/// \brief An enumerated integer property defining the way this camera transmits the data valid (\b DVAL)
			/// signal.
			///
...
#		endif // #ifdef DOTNET_ONLY_CODE
		...
		///\htmlinclude code_fragment_start_cpp.html
		///
		/// \b EXAMPLE(lists all available camera descriptions for a certain device):
		///\code
		/// #include <mvIMPACT_CPP/mvIMPACT_acquire.h>
		/// #include <algorithm>
		/// #include <iostream>
...
#			endif // #ifdef DOXYGEN_SHOULD_SKIP_THIS
		...
		public:
			...
			explicit CameraDescriptionManager(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::>DeviceManager object.
												Device* pDev ) : m_pRefData(0)
			{
				if( !pDev->isOpen() )
				{
			...
			/// \brief Constructs a new \b mvIMPACT::acquire::CameraDescriptionManager from
			/// and exisiting one.
...
#			endif // #ifndef WRAP_PYTHON (In Python, object assignment amounts to just a reference count increment anyhow; you need to call the constructor or possibly some slice operation to make a true copy)
			...
			/// \brief Returns a pointer to a \b mvIMPACT::acquire::CameraDescriptionCameraLink object specifying the camera
			...
			/// \brief Returns a pointer to a \b mvIMPACT::acquire::CameraDescriptionSDI object specifying the camera
			/// description found at the given index in the camera description managers internal list.
			///
			///\xmlonly
			/// <mvWrapperDoc><htmlinclude ref="code_fragment_start_cpp.html"/></mvWrapperDoc>
			...
			/// \brief Returns a pointer to a \b mvIMPACT::acquire::CameraDescriptionDigital object specifying the camera
			/// description found at the given index in the camera description managers internal list.
			///
			///\xmlonly
			/// <mvWrapperDoc><htmlinclude ref="code_fragment_start_cpp.html"/></mvWrapperDoc>
			...
			/// \brief Returns a pointer to a \b mvIMPACT::acquire::CameraDescriptionStandard object specifying the camera
			/// description found at the given index in the camera description managers internal list.
			///
			///\xmlonly
			/// <mvWrapperDoc><htmlinclude ref="code_fragment_start_cpp.html"/></mvWrapperDoc>
			...
			/// \brief Returns a pointer to a \b mvIMPACT::acquire::CameraDescriptionNonStandard object specifying the camera
			/// description found at the given index in the camera description managers internal list.
			///
			///\xmlonly
			/// <mvWrapperDoc><htmlinclude ref="code_fragment_start_cpp.html"/></mvWrapperDoc>
...
#	ifndef IGNORE_MVBLUEFOX_SPECIFIC_DOCUMENTATION
		...
		public:
			...
			explicit I2CControl(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
...
#	endif // #ifndef IGNORE_MVBLUEFOX_SPECIFIC_DOCUMENTATION
	...
	public:
		...
		explicit MotorFocusControl(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
...
#	ifndef IGNORE_MVBLUEFOX_SPECIFIC_DOCUMENTATION
		...
		public:
			...
			explicit DigitalIOMeasurementControl(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
...
#	endif // #ifndef IGNORE_MVBLUEFOX_SPECIFIC_DOCUMENTATION
	...
	/// device(\b Device specific interface layout only).
	///
	...
	{
	public:
		explicit FullSettingsBase(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
									Device* pDev,
									/// The name of the driver internal setting to access with this instance.
									/// A list of valid setting names can be obtained by a call to
									/// \b mvIMPACT::acquire::FunctionInterface::getAvailableSettings, new
...
#	ifndef IGNORE_MVBLUEFOX_SPECIFIC_DOCUMENTATION
		...
		/// which are unique for the \b mvBlueFOX(\b Device specific interface layout only).
		///
		...
		{
		public:
			explicit SettingsBlueFOX(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
										Device* pDev,
										/// The name of the driver internal setting to access with this instance.
										/// A list of valid setting names can be obtained by a call to
										/// \b mvIMPACT::acquire::FunctionInterface::getAvailableSettings, new
...
#	ifndef IGNORE_MVGRABBER_SPECIFIC_DOCUMENTATION
		//-----------------------------------------------------------------------------
		/// \brief This class provides access to general settings as well as to settings
		/// which are unique for frame grabber devices(\b Device specific interface layout only).
		///
		/// To see a small code example on how you can work with object of this class see
		/// the detailed description of the class \b mvIMPACT::acquire::FunctionInterface as well.
		/// Statements made for the use of the class \b mvIMPACT::acquire::FullSettingsBase apply for
		...
		{
		public:
			explicit SettingsFrameGrabber(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
											Device* pDev,
											/// The name of the driver internal setting to access with this instance.
											/// A list of valid setting names can be obtained by a call to
											/// \b mvIMPACT::acquire::FunctionInterface::getAvailableSettings, new
...
#	ifndef IGNORE_MVBLUEFOX_SPECIFIC_DOCUMENTATION
		//-----------------------------------------------------------------------------
		/// \brief A class containing \b mvBlueFOX specific settings to control the overall
		/// behaviour of the driver(\b Device specific interface layout only).
		///
		/// \note This class will only be available if \b mvIMPACT::acquire::Device::interfaceLayout is set to
		/// \b mvIMPACT::acquire::dilDeviceSpecific before the device is opened.
		...
		public:
			...
			explicit SystemBlueFOX(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.
...
#	ifndef IGNORE_MVV4L2_SPECIFIC_DOCUMENTATION
		//-----------------------------------------------------------------------------
		/// \brief A class containing \b mvV4L2 specific settings to control the overall
		/// behaviour of the driver(\b Device specific interface layout only).
		///
		/// \note UNDER CONSTRUCTION! Subject to change.
		///
		...
		public:
			...
			explicit SystemV4L2(	/// A pointer to a \b mvIMPACT::acquire::Device object obtained from a \b mvIMPACT::acquire::DeviceManager object.

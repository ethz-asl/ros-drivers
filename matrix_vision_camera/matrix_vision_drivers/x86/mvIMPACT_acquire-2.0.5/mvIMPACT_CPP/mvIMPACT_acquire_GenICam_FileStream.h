...
public:
						...
						std::ios_base::openmode mode )
	{
		...
		std::streambuf::setp( m_pBuffer, m_pBuffer + bufSize );
		return this;
	}
	/// \brief Determines if a file is open.
	///
...
protected:
	std::streamsize xsputn( const char_type* s, std::streamsize n )
	{
		...
		else
		{
			...
			{
				if( traits_type::eq_int_type( std::streambuf::sputc( s[i] ), traits_type::eof() ) )
				{
					return i;
				}
			}
	...
	int_type overflow( int_type c = traits_type::eof() )
	{
		...
		{
			return std::streambuf::sputc( static_cast<char_type>(c) );
		}
		return traits_type::not_eof( c );
	}
	int sync( void )
